"""Remote READ-ONLY provider Tool dispatch (Phase 13).
...
(`.literature.search`), never by the provider key prefix — the prefix is dynamic
provider data, and Core must not branch on provider vocabulary. It is the same
single-sourcing pattern as `revolab.tools.explicit_actions` for remote explicit
actions.
...
from revolab.schemas import LiteratureSearchToolInput
