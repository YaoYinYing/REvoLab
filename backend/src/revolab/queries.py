...
def bounded_graph(
    ...
    if "evidence" in kinds:
        for evidence in session.scalars(select(Evidence).where(Evidence.project_id == project_id)):
            ...
            if source_reachable or target_reachable:
                edges.append(
                    {
                        "kind": "evidence",
                        "id": str(evidence.id),
...
def generated_by(session: Session, project_id: UUID, revision_id: UUID) -> list[dict[str, Any]]:
    ...
    <-produced<- Run/Session. Returns the producing run/session chain."""
    visible = visible_resources(session, project_id)
...
def reference_summary(session: Session, resource_id: UUID, kind: ResourceKind) -> dict[str, Any]:
    ...
    if kind is ResourceKind.RUN_REFERENCE:
        row = session.get(RunReference, resource_id)
        if row is None:
            raise NotFoundError("run reference not found")
        base["authority"] = row.authority
        base["native_id"] = row.native_id
        base["task_type"] = row.task_type
        base["created_at"] = row.created_at
        base["revoked_at"] = row.revoked_at
