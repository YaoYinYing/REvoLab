...
| 5 | ScientificObject | `consumed_as_input_by` | RunReference \| SessionReference | 0..* | yes |
...
```text
Evidence.source  (exactly one, required) — what was interpreted
...
- **Cardinality:** exactly one `source` and one `target` per Evidence. A many-sided
