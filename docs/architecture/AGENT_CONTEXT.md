...
> **Status:** Accepted. Defines the Agent as a *consumer* (never an owner) of project
...
```text
ObjectRef   {type, id, provider?, external_id?, label}
EvidenceRef {id, kind, provider, external_id, role/polarity}
...
## The promotion loop (chat never becomes project truth)
...
→ persisted project truth  (ONLY the validated domain write lands in the DB)
```
...
Unvalidated prose is never persisted as truth.
...
| **Domain mutation** (create/update a Note draft, attach evidence, create an object, record a Decision, link a Relation) | Automatic OR project-policy | typed domain command + domain validation (ownership, project-belonging, schema) |
